...
const JobLogs: React.FC = () => {
  ...
  const fetchLogCount = async (): Promise<number | null> => {
    ...
    try {
      ...
      if (
        countResponse.status_code === 200 &&
        "count" in countResponse.response
      ) {
  ...
  const loadMoreLogs = async () => {
    ...
    try {
      ...
      const currentOffset = Math.max(
        0,
        totalLogCount - logs.length - LOAD_MORE_COUNT,
      );
      const logsResponse = await listJobLogs({
        job_id: parseInt(jobId),
    ...
    try {
      ...
      if (
        ...
        if (newTotalCount > totalLogCount) {
          ...
          if (
            logsResponse.status_code === 200 &&
            "logs" in logsResponse.response
          ) {
            const newLogs = logsResponse.response.logs;
            ...
            {logs.length === 0 ? (
              ...
              logs.map((log) => (
                ...
                >
                  ...
                  <Box
                    sx={{ flex: 1, color: "#f0f6fc", whiteSpace: "pre-wrap" }}
                  >
                    {log.message}
                  </Box>
                </Box>
